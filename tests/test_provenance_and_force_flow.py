...
def test_accepted_generated_text_is_marked(env):
    ...
    assert st["caption"].startswith("AI-generated by Claude Code")
    assert "No Commissioner edits." in st["caption"]


def test_one_edited_character_retires_the_claim(env):
    """The whole design. Nothing has to notice the edit or clean up after
    it: the hash simply stops matching."""
    _c, db, _ed = env
    with Storage(db) as s:
        _rec(s, "Generated prose.")
        assert _state(s, "Generated prose!") is None
        assert _state(s, "Generated prose. ") is not None   # whitespace only
...
def test_deterministic_output_does_not_wear_an_ai_badge():
    ...
    assert st["generator"] is None
    assert "Claude" not in st["caption"] and "ChatGPT" not in st["caption"]
    assert st["caption"].startswith("Generated automatically")
    assert "deterministic transaction analysis" in st["caption"]
...
def test_the_badge_is_not_a_logo():
    ...
    tpl = pathlib.Path("templates/public/_ai_provenance.html").read_text(encoding="utf-8")
...
def test_provenance_is_frozen_into_the_published_snapshot(env):
    """The claim is about a particular text, so it belongs beside that text
    rather than being looked up later against a database that has moved."""
    import inspect

    from leaguepage import publish

    src = inspect.getsource(publish.publish_assembled_issue)
    assert "provenance.state_for" in src
